  ...
  function apiResolverService($q, $log, api) {
    ...
    function resolve(action, parameters){
      var actionParts = action.split('@'),
          ...
          params = parameters || {};
      if(!resource || !method){
        $log.error('apiResolver.resolve requires correct action parameter');
        return false;
      }
      ...
      if(!apiObject){
        $log.error('Resource "' + resource + '" is not defined in the api services!');
        deferred.reject('Resource "' + resource + '" is not defined in the api services!');
      }else{
